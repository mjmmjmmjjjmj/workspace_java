...
//--- 스택에서 데이터를 피크(peek, 정상에 있는 데이터를 들여다봄) ---//
	public int peek() throws EmptyIntStackException {
		...
		return stk[ptr-1]; //스택 정상에 있는 데이터를 반환
		...
		while (true) {
			...
			case 4: // 덤프
				try {
					...
					System.out.println("비어있는 스택" + e.getMessage());
					e.printStackTrace();
				}
			...
			case 5: //clear
				try {
					s.clear();
					System.out.println("스택이 비워짐");
				} catch (IntStack5.EmptyIntStackException e) {
					System.out.println("스택이 이미 비어있음" + e.getMessage());
					e.printStackTrace();
				}
				break;
			}
